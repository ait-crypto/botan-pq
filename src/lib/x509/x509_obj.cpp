...
namespace Botan {
  ...
  namespace {
    ...
    Pss_params decode_pss_params(const std::vector<uint8_t>& encoded_pss_params) {
      ...
      BER_Decoder(encoded_pss_params)
          .start_sequence()
          .decode_optional(pss_parameter.hash_algo, ASN1_Type(0),
                           ASN1_Class::ExplicitContextSpecific, default_hash)
          .decode_optional(pss_parameter.mask_gen_algo, ASN1_Type(1),
                           ASN1_Class::ExplicitContextSpecific, default_mgf)
          .decode_optional(pss_parameter.salt_len, ASN1_Type(2),
                           ASN1_Class::ExplicitContextSpecific, size_t(20))
          .decode_optional(pss_parameter.trailer_field, ASN1_Type(3),
                           ASN1_Class::ExplicitContextSpecific, size_t(1))
          .end_cons();
  ...
  std::string X509_Object::hash_used_for_signature() const {
    ...
    if (sig_info[1] == "EMSA4") {
      const OID hash_oid =
          decode_pss_params(signature_algorithm().get_parameters()).hash_algo.get_oid();
      return hash_oid.to_formatted_string();
    } else {
      const std::vector<std::string> pad_and_hash = parse_algorithm_name(sig_info[1]);
  ...
  Certificate_Status_Code X509_Object::verify_signature(const Public_Key& pub_key) const {
    ...
    if (sig_info.size() == 2)
      padding = sig_info[1];
             ...
             pub_key_algo.find("Picnic") != std::string::npos ||
             pub_key_algo.find("Dilithium") != std::string::npos ||
             pub_key_algo.find("SPHINCS+") != std::string::npos)
      padding = "Pure";
    else
      return Certificate_Status_Code::SIGNATURE_ALGO_BAD_PARAMS;
    ...
    if (padding == "EMSA4") {
      ...
      if (hash_algo != "SHA-160" && hash_algo != "SHA-224" && hash_algo != "SHA-256" &&
          hash_algo != "SHA-384" && hash_algo != "SHA-512") {
      ...
      // For MGF1, it is strongly RECOMMENDED that the underlying hash function be the same as the
      // one identified by hashAlgorithm Must be SHA1, SHA2-224, SHA2-256, SHA2-384 or SHA2-512
      ...
      padding +=
          "(" + hash_algo + "," + mgf_algo + "," + std::to_string(pss_parameter.salt_len) + ")";
    } else {
      /*
   ...
   */
  std::vector<uint8_t> X509_Object::make_signed(PK_Signer* signer, RandomNumberGenerator& rng,
  ...
  namespace {
    ...
    std::string choose_sig_algo(AlgorithmIdentifier& sig_algo, const Private_Key& key,
                                const std::string& hash_fn, const std::string& user_specified) {
      ...
      if (algo_name == "RSA") {
      ...
      } else if (algo_name == "DSA" || algo_name == "ECDSA" || algo_name == "ECGDSA" ||
                 algo_name == "ECKCDSA" || algo_name == "GOST-34.10" ||
                 algo_name == "GOST-34.10-2012-256" || algo_name == "GOST-34.10-2012-512") {
        padding = "EMSA1(" + hash_fn + ")";
      } else if (algo_name == "Ed25519" || algo_name.find("Picnic") != std::string::npos ||
                 algo_name.find("Dilithium") != std::string::npos ||
                 algo_name.find("SPHINCS+") != std::string::npos) {
        padding = "Pure";
        ...
        sig_algo =
            AlgorithmIdentifier(OID::from_string("XMSS"), AlgorithmIdentifier::USE_EMPTY_PARAM);
        return padding;
      } else {
      ...
      if (padding != "Pure") {
        ...
        sig_algo =
            AlgorithmIdentifier(OID::from_string(algo_name), AlgorithmIdentifier::USE_EMPTY_PARAM);
        return "Pure";
      }
    }
