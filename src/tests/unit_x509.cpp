...
namespace Botan_Tests {
  ...
  namespace {
    ...
    Botan::X509_Cert_Options req_opts1(const std::string& algo,
      ...
      if (algo == "RSA") {
        opts.constraints = Botan::Key_Constraints(Botan::KEY_ENCIPHERMENT);
                 ...
                 algo.find("Picnic") != std::string::npos ||
                 algo.find("Dilithium") != std::string::npos ||
                 algo.find("SPHINCS+") != std::string::npos) {
    ...
    Test::Result test_cert_status_strings() {
      ...
      result.test_eq("Same string", Botan::to_string(Botan::Certificate_Status_Code::OK),
    ...
    Test::Result test_x509_extension() {
      ...
      result.test_eq("Extension::get_extension_bits", extn.get_extension_bits(oid_bc),
                     "30060101FF020100");
      ...
      result.confirm(
          "Returns false since extension already existed",
      ...
      result.confirm("Replaced basic constraints is not critical",
                     !extn.critical_extension_set(oid_bc));
    ...
    Test::Result test_x509_dates() {
      ...
      result.test_eq("UTC_OR_GENERALIZED_TIME from UTC_TIME readable_string",
                     time.readable_string(), "2020/03/05 10:03:50 UTC");
      ...
      result.test_eq("GENERALIZED_TIME readable_string", time.readable_string(),
                     "2020/03/05 10:03:50 UTC");

      // Dates that are valid per X.500 but rejected as unsupported
      const std::string valid_but_unsup[]{
          "0802010000-0000",   "0802011724+0000",   "0406142334-0500",
          "9906142334+0500",   "0006142334-0530",   "0006142334+0530",

          "080201000000-0000", "080201172412+0000", "040614233433-0500",
          "990614233444+0500", "000614233455-0530", "000614233455+0530",
      };

      // valid length 13
      const std::string valid_utc[]{
          "080201000000Z", "080201172412Z", "040614233433Z", "990614233444Z", "000614233455Z",
      };
      ...
      for (const auto& v : valid_but_unsup) {
        result.test_throws("valid but unsupported",
                           [v]() { Botan::X509_Time t(v, Botan::ASN1_Type::UtcTime); });
      }
      ...
      for (const auto& v : invalid_generalized) {
        result.test_throws("invalid",
                           [v]() { Botan::X509_Time t(v, Botan::ASN1_Type::GeneralizedTime); });
      }
    ...
    Test::Result test_rsa_oaep() {
      ...
      result.test_eq("RSA-OAEP OID", pk_info.get_oid().to_string(),
                     Botan::OID::from_string("RSA/OAEP").to_string());
#endif
    ...
    Test::Result test_x509_decode_list() {
      ...
      result.test_eq("Expected cert 1 CN", certs[0].subject_dn().get_first_attribute("CN"),
                     "CA1-PP.01.02");
      result.test_eq("Expected cert 2 CN", certs[1].subject_dn().get_first_attribute("CN"),
                     "User1-PP.01.02");
    ...
    Test::Result test_x509_utf8() {
      ...
      try {
        ...
        const std::string organization = "\xD0\x9C\xD0\xBE\xD1\x8F\x20\xD0\xBA\xD0\xBE\xD0"
        ...
        const std::string common_name = "\xD0\x9E\xD0\xBF\xD0\xB8\xD1\x81\xD0\xB0\xD0\xBD\xD0\xB8"
    ...
    Test::Result test_x509_authority_info_access_extension() {
      ...
      Botan::X509_Certificate aia_cert(
          Test::data_file("x509/misc/contains_authority_info_access.pem"));
      ...
      result.test_eq("OCSP responder URL matches", aia_cert.ocsp_responder(),
                     "http://gp.symcd.com");
      ...
      result.test_eq("CA issuer URL matches", ca_issuers2[0],
                     "http://www.d-trust.net/cgi-bin/Bdrive_Test_CA_1-2_2017.crt");
      result.test_eq(
          "CA issuer URL matches", ca_issuers2[1],
          "ldap://directory.d-trust.net/"
          "CN=Bdrive%20Test%20CA%201-2%202017,O=Bundesdruckerei%20GmbH,C=DE?cACertificate?base?");
      result.test_eq("OCSP responder URL matches", aia_cert_2ca.ocsp_responder(),
 ...
 */
    ...
    Test::Result test_padding_config() {
      ...
      Botan::X509_Certificate ca_cert_def =
          Botan::X509::create_self_signed_cert(opt, (*sk), "SHA-512", Test::rng());
      test_result.test_eq(
          "CA certificate signature algorithm (default)",
          Botan::OIDS::oid2str_or_throw(ca_cert_def.signature_algorithm().get_oid()),
          "RSA/EMSA3(SHA-512)");

      // Create X509 CA certificate; RSA-PSS is explicitly set
      opt.set_padding_scheme("PSSR");
      Botan::X509_Certificate ca_cert_exp =
          Botan::X509::create_self_signed_cert(opt, (*sk), "SHA-512", Test::rng());
      test_result.test_eq(
          "CA certificate signature algorithm (explicit)",
      ...
      try {
        Botan::X509_Certificate ca_cert_wrong =
            Botan::X509::create_self_signed_cert(opt, (*sk), "SHA-512", Test::rng());
        test_result.test_failure(
            "Could build CA cert with invalid encoding scheme EMSA1 for key type " +
            sk->algo_name());
      } catch (const Botan::Invalid_Argument& e) {
        test_result.test_eq(
            "Build CA certitiface with invalid encoding scheme EMSA1 for key type " +
                sk->algo_name(),
            e.what(),
      ...
      Botan::PKCS10_Request end_req =
          Botan::X509::create_cert_req(req_opt, (*sk), "SHA-512", Test::rng());
      test_result.test_eq("Certificate request signature algorithm",
                          Botan::OIDS::oid2str_or_throw(end_req.signature_algorithm().get_oid()),
                          "RSA/EMSA4");

      // Create X509 CA object: will fail as the chosen hash functions differ
      try {
        Botan::X509_CA ca_fail(ca_cert_exp, (*sk), {{"padding", "EMSA4(SHA-256)"}}, "SHA-512",
                               Test::rng());
        test_result.test_failure("Configured conflicting hash functions for CA");
      } catch (const Botan::Invalid_Argument& e) {
        test_result.test_eq("Configured conflicting hash functions for CA", e.what(),
                            "Hash function from opts and hash_fn argument need to be identical");
      }

      // Create X509 CA object: its signer will use the padding scheme from the CA certificate, i.e.
      // EMSA3
      Botan::X509_CA ca_def(ca_cert_def, (*sk), "SHA-512", Test::rng());
      Botan::X509_Certificate end_cert_emsa3 =
          ca_def.sign_request(end_req, Test::rng(), not_before, not_after);
      test_result.test_eq(
          "End certificate signature algorithm",
          Botan::OIDS::oid2str_or_throw(end_cert_emsa3.signature_algorithm().get_oid()),
          "RSA/EMSA3(SHA-512)");

      // Create X509 CA object: its signer will use the explicitly configured padding scheme, which
      // is different from the CA certificate's scheme
      Botan::X509_CA ca_diff(ca_cert_def, (*sk), {{"padding", "EMSA-PSS"}}, "SHA-512", Test::rng());
      Botan::X509_Certificate end_cert_diff_emsa4 =
          ca_diff.sign_request(end_req, Test::rng(), not_before, not_after);
      test_result.test_eq(
          "End certificate signature algorithm",
          Botan::OIDS::oid2str_or_throw(end_cert_diff_emsa4.signature_algorithm().get_oid()),
          "RSA/EMSA4");

      // Create X509 CA object: its signer will use the explicitly configured padding scheme, which
      // is identical to the CA certificate's scheme
      Botan::X509_CA ca_exp(ca_cert_exp, (*sk), {{"padding", "EMSA4(SHA-512,MGF1,64)"}}, "SHA-512",
                            Test::rng());
      Botan::X509_Certificate end_cert_emsa4 =
          ca_exp.sign_request(end_req, Test::rng(), not_before, not_after);
      test_result.test_eq(
          "End certificate signature algorithm",
          Botan::OIDS::oid2str_or_throw(end_cert_emsa4.signature_algorithm().get_oid()),
          "RSA/EMSA4");

      // Check CRL signature algorithm
      Botan::X509_CRL crl = ca_exp.new_crl(Test::rng());
      test_result.test_eq("CRL signature algorithm",
                          Botan::OIDS::oid2str_or_throw(crl.signature_algorithm().get_oid()),
                          "RSA/EMSA4");
      ...
      test_result.confirm("EMSA4-signed certificate validates",
    ...
    Test::Result test_pkcs10_ext(const Botan::Private_Key& key, const std::string& sig_padding,
      ...
      opts.extensions.add(
          std::make_unique<Botan::Cert_Extension::Subject_Alternative_Name>(alt_name));
    ...
    Test::Result test_x509_cert(const Botan::Private_Key& ca_key, const std::string& sig_algo,
                                const std::string& sig_padding = "",
                                const std::string& hash_fn     = "SHA-256") {
      ...
      Botan::PKCS10_Request user1_req = Botan::X509::create_cert_req(
          req_opts1(sig_algo, sig_padding), *user1_key, hash_fn, Test::rng());
      ...
      Botan::X509_Certificate user1_cert = ca.sign_request(
          user1_req, Test::rng(), user1_serial, from_date(-1, 01, 01), from_date(2, 01, 01));
      ...
      const auto user1_ss_cert = Botan::X509::create_self_signed_cert(
          req_opts1(sig_algo, sig_padding), *user1_key, hash_fn, Test::rng());

      {
        auto constrains = req_opts1(sig_algo).constraints;
        result.test_eq("user1 key usage", (user1_cert.constraints() & constrains) == constrains,
                       true);
      }
      ...
      result.test_eq("issuer info CN", user1_issuer_dn.get_first_attribute("CN"),
                     ca_opts().common_name);
      result.test_eq("issuer info Country", user1_issuer_dn.get_first_attribute("C"),
                     ca_opts().country);
      result.test_eq("issuer info Orga", user1_issuer_dn.get_first_attribute("O"),
                     ca_opts().organization);
      result.test_eq("issuer info OrgaUnit", user1_issuer_dn.get_first_attribute("OU"),
                     ca_opts().org_unit);

      const Botan::X509_DN& user3_subject_dn = user3_cert.subject_dn();
      result.test_eq("subject OrgaUnit count", user3_subject_dn.get_attribute("OU").size(),
                     req_opts3(sig_algo).more_org_units.size() + 1);
      result.test_eq("subject OrgaUnit #2", user3_subject_dn.get_attribute("OU").at(1),
                     req_opts3(sig_algo).more_org_units.at(0));

      const Botan::AlternativeName& user1_altname = user1_cert.subject_alt_name();
      result.test_eq("subject alt email", user1_altname.get_first_attribute("RFC822"),
                     "testing@randombit.net");
      result.test_eq("subject alt dns", user1_altname.get_first_attribute("DNS"),
                     "botan.randombit.net");
      result.test_eq("subject alt uri", user1_altname.get_first_attribute("URI"),
                     "https://botan.randombit.net");

      const Botan::AlternativeName& user3_altname = user3_cert.subject_alt_name();
      result.test_eq("subject alt dns count", user3_altname.get_attribute("DNS").size(),
                     req_opts3(sig_algo).more_dns.size() + 1);
      result.test_eq("subject alt dns #2", user3_altname.get_attribute("DNS").at(1),
                     req_opts3(sig_algo).more_dns.at(0));
      ...
      Botan::Path_Validation_Result result_no_issuer =
          Botan::x509_path_validate(user1_cert, restrictions, store);
      result.test_eq("user 1 issuer not found", result_no_issuer.result_string(),
                     Botan::Path_Validation_Result::status_string(
                         Botan::Certificate_Status_Code::CERT_ISSUER_NOT_FOUND));

      store.add_certificate(ca.ca_certificate());

      Botan::Path_Validation_Result result_u1 =
          Botan::x509_path_validate(user1_cert, restrictions, store);
      ...
      Botan::Path_Validation_Result result_u2 =
          Botan::x509_path_validate(user2_cert, restrictions, store);
      ...
      Botan::Path_Validation_Result result_self_signed =
      ...
      const std::string revoked_str = Botan::Path_Validation_Result::status_string(
          Botan::Certificate_Status_Code::CERT_IS_REVOKED);
    ...
    Test::Result test_usage(const Botan::Private_Key& ca_key, const std::string& sig_algo,
                            const std::string& hash_fn = "SHA-256") {
      ...
      const Botan::PKCS10_Request user1_req =
          Botan::X509::create_cert_req(opts, *user1_key, hash_fn, Test::rng());
      ...
      result.test_eq("key usage cRLSign not allowed",
                     ...
                     false);
      result.test_eq("encryption is not allowed", user1_cert.allowed_usage(Usage_Type::ENCRYPTION),
                     false);

      // cert only allows digitalSignature, so checking for only that should be ok
      result.confirm("key usage digitalSignature allowed",
                     user1_cert.allowed_usage(Key_Constraints::DIGITAL_SIGNATURE));
      ...
      result.confirm("key usage multiple cRLSign allowed",
                     mult_usage_cert.allowed_usage(Key_Constraints::CRL_SIGN));
      result.confirm("key usage multiple digitalSignature and cRLSign allowed",
                     mult_usage_cert.allowed_usage(Key_Constraints(
                         Key_Constraints::DIGITAL_SIGNATURE | Key_Constraints::CRL_SIGN)));
      ...
      result.confirm("key usage digitalSignature allowed",
                     no_usage_cert.allowed_usage(Key_Constraints::DIGITAL_SIGNATURE));
      result.confirm("key usage cRLSign allowed",
                     no_usage_cert.allowed_usage(Key_Constraints::CRL_SIGN));
      result.confirm("key usage encryption allowed",
                     no_usage_cert.allowed_usage(Usage_Type::ENCRYPTION));

      if (sig_algo == "RSA") {
        // cert allows data encryption
        opts.constraints =
            Key_Constraints(Key_Constraints::KEY_ENCIPHERMENT | Key_Constraints::DATA_ENCIPHERMENT);

        const Botan::PKCS10_Request enc_req =
            Botan::X509::create_cert_req(opts, *user1_key, hash_fn, Test::rng());
        ...
        result.confirm("cert does not allow TLS client auth",
                       !enc_cert.allowed_usage(Usage_Type::TLS_CLIENT_AUTH));
      }

      return result;
    }

    Test::Result test_self_issued(const Botan::Private_Key& ca_key, const std::string& sig_algo,
                                  const std::string& sig_padding = "",
                                  const std::string& hash_fn     = "SHA-256") {
      using Botan::Key_Constraints;
      ...
      const Botan::PKCS10_Request self_issued_req =
          Botan::X509::create_cert_req(opts, *user_key, hash_fn, Test::rng());
      ...
      result.confirm("chain with self-issued cert validates",
                     validation_result.successful_validation());
    ...
    Test::Result test_x509_uninit() {
      ...
      result.test_throws("uninitialized cert access causes exception",
                         "X509_Certificate uninitialized", [&cert]() { cert.x509_version(); });

      Botan::X509_CRL crl;
      result.test_throws("uninitialized crl access causes exception", "X509_CRL uninitialized",
                         [&crl]() { crl.crl_number(); });
    ...
    struct typical_usage_constraints {
      ...
      Key_Constraints all =
          Key_Constraints(Key_Constraints::DIGITAL_SIGNATURE | Key_Constraints::NON_REPUDIATION |
                          Key_Constraints::KEY_ENCIPHERMENT | Key_Constraints::DATA_ENCIPHERMENT |
                          Key_Constraints::KEY_AGREEMENT | Key_Constraints::KEY_CERT_SIGN |
                          Key_Constraints::CRL_SIGN | Key_Constraints::ENCIPHER_ONLY |
      ...
      Key_Constraints non_repudiation =
          Key_Constraints(Key_Constraints::NON_REPUDIATION | Key_Constraints::DIGITAL_SIGNATURE);
      ...
      Key_Constraints key_agreement_encipher_only =
          Key_Constraints(Key_Constraints::KEY_AGREEMENT | Key_Constraints::ENCIPHER_ONLY);
      Key_Constraints key_agreement_decipher_only =
          Key_Constraints(Key_Constraints::KEY_AGREEMENT | Key_Constraints::DECIPHER_ONLY);
      Key_Constraints crl_sign = Key_Constraints::CRL_SIGN;
      Key_Constraints sign_everything =
          Key_Constraints(Key_Constraints::DIGITAL_SIGNATURE | Key_Constraints::KEY_CERT_SIGN |
                          Key_Constraints::CRL_SIGN);
    };
    ...
    Test::Result test_valid_constraints(const Botan::Private_Key& key, const std::string& pk_algo) {
      ...
      verify_cert_constraints_valid_for_key_type(key,
                                                 Key_Constraints(Key_Constraints::NO_CONSTRAINTS));
      ...
      if (pk_algo == "DH" || pk_algo == "ECDH") {
        ...
        result.test_throws(
            "sign, cert sign, crl sign not permitted not permitted", [&key, &typical_usage]() {
              verify_cert_constraints_valid_for_key_type(key, typical_usage.sign_everything);
            });
      } else if (pk_algo == "DSA" || pk_algo == "ECDSA" || pk_algo == "ECGDSA" ||
                 pk_algo == "ECKCDSA" || pk_algo == "GOST-34.10" ||
                 pk_algo.find("Picnic") != std::string::npos ||
                 pk_algo.find("Dilithium") != std::string::npos ||
                 pk_algo.find("SPHINCS+") != std::string::npos) {
    ...
    public:
      ...
      std::vector<uint8_t> encode_inner() const override {
        ...
        Botan::DER_Encoder(bits).encode(
            Botan::ASN1_String(m_contents, Botan::ASN1_Type::Utf8String));
        return bits;
      }
    ...
    Test::Result test_custom_dn_attr(const Botan::Private_Key& ca_key, const std::string& sig_algo,
                                     const std::string& sig_padding = "",
                                     const std::string& hash_fn     = "SHA-256") {
      ...
      Botan::PKCS10_Request req = Botan::PKCS10_Request::create(*user_key, subject_dn, extensions,
                                                                hash_fn, Test::rng(), sig_padding);
    ...
    Test::Result test_x509_extensions(const Botan::Private_Key& ca_key, const std::string& sig_algo,
                                      const std::string& sig_padding = "",
                                      const std::string& hash_fn     = "SHA-256") {
      using Botan::Key_Constraints;
      ...
      if (result.confirm("Key_Usage extension present in self-signed certificate",
                         key_usage_ext != nullptr)) {
        result.confirm(
            "Key_Usage extension value matches in self-signed certificate",
            dynamic_cast<Botan::Cert_Extension::Key_Usage&>(*key_usage_ext).get_constraints() ==
                opts.constraints);
      }
      ...
      if (result.confirm("Custom extension present in self-signed certificate",
                         string_ext != nullptr)) {
        result.test_eq("Custom extension value matches in self-signed certificate",
                       string_ext->value(), "AAAAAAAAAAAAAABCDEF");
      }
      ...
      if (result.confirm("Key_Usage extension present in CA-signed certificate",
                         key_usage_ext != nullptr)) {
        result.confirm(
            "Key_Usage extension value matches in user certificate",
            dynamic_cast<Botan::Cert_Extension::Key_Usage&>(*key_usage_ext).get_constraints() ==
      ...
      if (result.confirm("Custom extension present in CA-signed certificate",
                         string_ext != nullptr)) {
    ...
    Test::Result test_hashes(const Botan::Private_Key& key,
      ...
      } const cases[]{{"", "", "E4F60D0AA6D7F3D3B6A6494B1C861B99F649C6F9EC51ABAF201B20F297327C95",
                       "E4F60D0AA6D7F3D3B6A6494B1C861B99F649C6F9EC51ABAF201B20F297327C95"},
                      {"a", "b", "BC2E013472F39AC579964880E422737C82BA812CB8BC2FD17E013060D71E6E19",
      ...
      for (const auto& a : cases) {
        ...
        result.test_eq(a.issuer, Botan::hex_encode(issuer_cert.raw_issuer_dn_sha256()),
                       a.issuer_hash);
        result.test_eq(a.issuer, Botan::hex_encode(issuer_cert.raw_subject_dn_sha256()),
                       a.issuer_hash);
        ...
        result.test_eq(a.subject, Botan::hex_encode(subject_cert.raw_issuer_dn_sha256()),
                       a.issuer_hash);
        result.test_eq(a.subject, Botan::hex_encode(subject_cert.raw_subject_dn_sha256()),
                       a.subject_hash);
      }
      return result;
    }
    ...
    public:
      std::vector<Test::Result> run() override {
        ...
        const std::string sig_algos[]{"RSA",     "DSA",        "ECDSA",   "ECGDSA",
                                      "ECKCDSA", "GOST-34.10", "Ed25519", "Picnic L1 FS"};
...
#if defined(BOTAN_TARGET_OS_HAS_FILESYSTEM) && defined(BOTAN_HAS_EMSA_PKCS1) &&                    \
    defined(BOTAN_HAS_EMSA_PSSR) && defined(BOTAN_HAS_RSA)
